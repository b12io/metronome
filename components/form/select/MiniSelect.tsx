...
import { Disabled } from "../../Icons.es6";
...
class MiniSelect extends React.Component<Props, State> {
  ...
  renderSelect = () => {
    ...
    return <div className={classNames} ref={(node: any) => {this.selectNode = node;}}>
        ...
        {isOpened && <ul className="ds-mini-select__dropdown-menu">
          {selectOptions.map(option => <li onClick={e => this.handleSelect(option)} key={option}>{option}</li>)}
        </ul>}
      </div>;
  };
  ...
  render() {
    ...
    return <div className={classnames('ds-form-group', 'ds-form-group--select', {
      'has-feedback': inputDisabled
        ...
        'ds-mini-select--disabled': selectDisabled
          ...
          <div className="ds-mini-select__input-wrap">
            <input className="ds-mini-select__input" disabled={inputDisabled} type="text" value={inputValue} onChange={e => inputValueChange(e.target.value)} placeholder={inputPlaceholder} />
