...
class FilterOptionSelect extends React.Component<Props> {
  ...
  render() {
    ...
    return <div className="ds-filter-option__select" onBlur={onBlur} ref={el => {this.dropdown = el;}} tabIndex="-1">
        <div className="ds-filter-option__select-header">
          Filter by...
        </div>
        {filterableFields.map(field => <div key={field} className="ds-filter-option__select-item" onClick={() => onFilterSelect(field)}>
            {deslugify(field)}
          </div>)}
      </div>;
  }
